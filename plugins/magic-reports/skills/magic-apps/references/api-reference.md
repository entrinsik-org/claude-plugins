...
**naturalId format:** Integration identifiers use `owner:slug` format (e.g., `thansen:grain`). In client-side code, use them as-is in the URL path. In server route `fetch()` calls, you must URL-encode the colon: `integrations/thansen%3Agrain/request`. See the [Server Routes section](../SKILL.md#using-fetch) for details.
