...
registry consumer reads — the Explorer app's grids, pivots, and charts,
exports, and SQL autocomplete. Declared once by you, resolved per viewer at
runtime, internationalized per locale.

**Version floor:** the semantic registry (`GET`/`PUT
/api/datasources/{id}/semantics`, the deploy extraction of `semantics.yaml`)
requires Informer **2026.2.0+**.
...
- `decimals` pins the fraction digits of a numeric, currency, or duration
  field; separators and date order stay with the viewer's locale.
- `expr` declares a computed field: one SQL expression over the table's own
  columns (`expr: '"qty" * "unit_price"'`), single statement by grammar.
...
The Explorer app's subject picker is a consumer surface: curate it.
Categories are **entities** with a visual identity; a table references one
by id:
...
tables:
  order_tasks:
    ...
    fields:
      sync_token: { hidden: true }   # plumbing — never reaches consumers
  _ingest_cursor:
    hidden: true            # retract the whole table from consumers
```
...
- `hidden: true` (structure file only — overlays can't hide) retracts a
  table or column from CONSUMER surfaces. Builder surfaces still show the
  whole schema; hidden is curation, not security.
- **Hiding a foreign-key column does NOT hide the relationship** — the
  Explorer still offers the hop to the target table; only the raw id column
...
## Exposure — open sources and curated sources

The registry resolves every relation and field with an `exposed` flag, and
one source-level policy decides the default:

```yaml
# semantics.yaml
exposure: curated     # or open
```

- **`open`** — everything the catalog (or a scan) knows is offered unless a
  row hides it. The default for catalog-backed engines such as Postgres.
- **`curated`** — scanned inventory is NOT offered until a semantics row
  describes it. Naming a field is what exposes it; an explicit `hidden`
  still wins either way. The default for MultiValue drivers, where one
  attribute can carry dozens of synonym dictionary items and the picker
  must show a single "First Name".

Omit the key to inherit the driver default. A customer can override the
policy from the Modeler app (it lands as a tenant policy row), and can
expose more of a curated source on top of yours without touching your
files. Consumers apply the same rule to entries the registry has never
seen: offered under `open`, held back under `curated`.

## Histogram bands — bins

Grouping a numeric field in the Explorer app builds a histogram automatically: an
...
- **Views need links.** A view carries no FK constraints — without a
  declared link its columns can't reach related tables in the Explorer.
...
- `one: true` asserts the `from` column is one-per-target: the reverse
  hop then traverses as REAL fields. The Explorer compiles it with a row
...
The registry resolves three layers, top wins per key, each key tagged with
its provenance: the customer's **tenant** rows over your **vendor** rows
over the **scan** layer — what Informer already knows about a source from
its own scan, such as labels curated in the datasource admin. Customers can
overlay your semantics (relabel a column for their org) and, in a later
release, define their own SQL views over your schema — including **shadow
views** that replace a table for their BI users (their column-level
security). Two consequences for you as the vendor:
