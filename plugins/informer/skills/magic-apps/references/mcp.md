...
- **A tool listed with no input schema** means its `export const schema` failed to evaluate at deploy (the log says `schema failed to evaluate`). The scanner evaluates the literal on its own, with no imports in scope — `enum: STATUS_OPTIONS` from an imported constant is the classic case. Inline the values; pin them to the shared constant with a test if they must stay in sync. Same rule for `export const description` (a plain string literal) — see the Handler Config section in `server-routes.md`.
